...
class ProfileController extends Controller
{
    ...
    public function index()
    {
        ...
        // Load service provider with relationships - FIXED: use 'foodConfig' instead of 'foodServiceConfig'
        $serviceProvider = $user->serviceProvider()->with(['foodConfig', 'mealTypes'])->firstOrFail();
        
        $foodConfig = $serviceProvider->foodConfig ?? null;
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $foodConfigData = [
                ...
                'subscription_discount_percent' => $request->subscription_discount_percent ?? 10.00,
            ];
            
            \Log::info('Food config data:', $foodConfigData);
            
            // FIXED: Use 'foodConfig' relationship instead of 'foodServiceConfig'
            if ($serviceProvider->foodConfig) {
                $foodConfigUpdated = $serviceProvider->foodConfig()->update($foodConfigData);
